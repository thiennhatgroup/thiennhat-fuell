-- ============================================================================
-- 0005_admin_users.sql — Admin tạo tài khoản + gán vai trò ngay trong app.
-- Auth user tạo thẳng trong DB với mật khẩu = 'tn-pin::<PIN>'; email_confirmed_at
-- được set để đăng nhập ngay. Gated 'user:manage' (Admin bỏ qua như '*').
-- ============================================================================
...
begin
  ...
  if p_role not in ('ThuKho','KeToan','Admin') then raise exception 'Vai trò không hợp lệ.'; end if;
...
begin
  ...
  if p_role is not null and p_role not in ('ThuKho','KeToan','Admin') then raise exception 'Vai trò không hợp lệ.'; end if;
